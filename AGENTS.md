...
Fork deployed as an SMS gateway for **smsandroid.com.br**. Stack:

- **API**: GCP Cloud Run `http-sms-api` (us-east1), image built via Cloud Build (kaniko). Domain `api.smsandroid.com.br`.
- **Web**: Firebase Hosting site `httpsms-prod`, SPA built with Nuxt 4 `pnpm run generate`. Domain `smsandroid.com.br`.
- **DB**: Neon PostgreSQL Serverless (AWS `sa-east-1` São Paulo, `neondb`), connected directly via SSL (`sslmode=require`).
- **Redis**: Upstash Redis Serverless TLS (`honest-silkworm-128513.upstash.io:6379`, AWS `sa-east-1` São Paulo), connected via TLS `rediss://`.
- **Events queue**: Cloud Tasks queue `httpsms-events` (us-east1), full path `projects/479893056966/locations/us-east1/queues/httpsms-events`. `EVENTS_QUEUE_TYPE=cloud-task`, `EVENTS_QUEUE_ENDPOINT=https://api.smsandroid.com.br/v1/events`.
...
Key prod env vars for the Cloud Run service (all on the service, secrets via Secret Manager `--set-secrets`): `ENV=production`, `GCP_PROJECT_ID=httpsms-prod`, `APP_HOST=0.0.0.0`, `APP_PORT=8000`, `SWAGGER_HOST=api.smsandroid.com.br`, `APP_URL=https://smsandroid.com.br`, `EVENTS_QUEUE_NAME=<full tasks path>`, `FIREBASE_CREDENTIALS`, `DATABASE_URL`/`DATABASE_URL_DEDICATED` (Neon PostgreSQL), `REDIS_URL` (Upstash TLS), `EVENTS_QUEUE_USER_ID`/`EVENTS_QUEUE_USER_API_KEY`, `SMTP_*` (mailtrap placeholder; optional for SMS core), `GCS_BUCKET_NAME=` (empty → in-memory MMS), `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`.
...
- The web `.env.production` was rewritten for prod (`API_BASE_URL=https://api.smsandroid.com.br`, `APP_URL=https://smsandroid.com.br`, Firebase config for `httpsms-prod`); upstream CHECKOUT/PUSHER/TURNSTILE vars removed (nuxt.config uses `|| ''` fallbacks).
...
Before committing and deploying any changes (`git push origin main` or triggering Cloud Run / Firebase Hosting deploys), you **MUST** run and pass all local builds, linting, and tests to prevent broken releases:

1. **API Validation** (run from `api/`):
   - `go test ./...` — all unit tests must pass with 0 failures.
   - `go build -o /dev/null .` — ensure the Go application compiles cleanly without missing methods, fields, or parameters.

2. **Web Validation** (run from `web/`):
   - `pnpm lint` — eslint, stylelint, and prettier must pass with 0 errors. (Run `pnpm lintfix` if formatting fixes are needed).
   - `NUXT_TELEMETRY_DISABLED=1 pnpm run generate` — static production build must generate all routes with 0 errors.

3. **Android Validation** (if Android files are modified):
   - Verify string resources and default URLs (`strings.xml`, `Settings.kt`).

**NEVER push or deploy without executing and passing these verification steps first.**

...
func newEntitlementServiceForTest(enabled bool, repository repositories.UserRepository) *EntitlementService {
	...
	return NewEntitlementService(logger, telemetry.NewOtelLogger("test", logger), enabled, repository, "http://localhost:3000")
}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !test.allowed {
				...
				assert.Contains(t, result.Message, "Upgrade to a paid plan")
			}
		})
	}
}
